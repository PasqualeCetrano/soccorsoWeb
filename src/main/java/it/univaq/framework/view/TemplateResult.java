 ...
 * 
 */
package it.univaq.framework.view;
...
public class TemplateResult {
    ...
    private void init() {
        ...
        cfg.setOutputEncoding(encoding); // serve per la codifica quando io scrivo sul browser
        cfg.setDefaultEncoding(encoding); // serve per indicare il formato per leggere i file dal server
        ...
        if (context.getInitParameter("view.template_directory") != null) {
            ...
            // context.getInitParameter("view.template_directory"));
            cfg.setTemplateLoader(new freemarker.ext.jakarta.servlet.WebappTemplateLoader(context,
                    context.getInitParameter("view.template_directory"))); // patch se usato con JakartaEE
        } else {
            // cfg.setServletContextForTemplateLoading(context, "templates");
            cfg.setTemplateLoader(new freemarker.ext.jakarta.servlet.WebappTemplateLoader(context, "templates")); // patch
                                                                                                                  // se
                                                                                                                  // usato
                                                                                                                  // con
                                                                                                                  // JakartaEE
    ...
    protected Map getDefaultDataModel(HttpServletRequest request) {
        ...
        // aggiungiamo altri dati di inizializzazione presi dal web.xml, prende tutti i
        // valori che iniziano con view.data.static.
    ...
    protected void process(String tplname, Map datamodel, HttpServletRequest request, Writer out)
        ...
        // outline_tpl key
        if (datamodel != null) {
            localdatamodel.putAll(datamodel); // mette datamodel servlet nel datamodellocale
        }
        String outline_name = (String) localdatamodel.get("outline_tpl"); // se non esiste otuline_tpl ho null come
                                                                          // valore restituito
        try {
            ...
            t.process(localdatamodel, out); // qui carica la pagina e sostituisce i tag con le variabili presenti nel
                                            // datamodel
    ...
    public void activate(String tplname, HttpServletRequest request, HttpServletResponse response)
        ...
        setupServletResponse(datamodel, response);// qui preparo la risposta
        try {
    ...
    private void setupServletResponse(Map datamodel, HttpServletResponse response) {
        ...
        response.setCharacterEncoding(encoding); // serve per impostare la codifica da mandare sulla rete per il browser

        // impostiamo il tipo di output: in questo modo freemarker abiliterà il
        // necessario escaping, serve per la sicurezza permette di eliminare i caratteri
        // speciali
