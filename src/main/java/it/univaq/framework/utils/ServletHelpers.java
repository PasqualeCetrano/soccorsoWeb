package it.univaq.framework.utils;

import it.univaq.framework.view.HTMLResult;
...
public class ServletHelpers {
    ...
    public static void handleError(String message, HttpServletRequest request, HttpServletResponse response,
        ...
        try {
            ...
            // se non possiamo inviare la pagina di errore, proviamo un messaggio di errore
            // HTTP standard
            try {
                response.sendError(HttpServletResponse.SC_INTERNAL_SERVER_ERROR, message);
            } catch (IOException ex1) {
                // if ALSO this error status cannot be notified, write to the server log
                // se ANCHE questo stato di errore non può essere notificato, scriviamo sul log
                // del server
    ...
    public static void handleError(Exception exception, HttpServletRequest request, HttpServletResponse response,
            ServletContext context) {
