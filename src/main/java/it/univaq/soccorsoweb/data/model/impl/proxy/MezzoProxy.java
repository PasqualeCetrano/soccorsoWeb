 ...
 * MezzoImpl e per modificare i valori degli attributi dell'oggetto Mezzo salvato in RAM,
 * ci basiamo sul richiamare i metodi presenti in MezzoImpl all'interno dei metodi 
 * ridefiniti nel proxy.
 *
 * richiamiamo i metodi in MezzoImpl perchè poichè quegli attributi sono dichiarati private,
...
public class MezzoProxy extends MezzoImpl implements DataItemProxy {
    ...
    public List<Missione> getMissioni() {
        ...
        if (super.getMissioni() == null && getKey() != null && getKey() > 0) {
            try {
                super.setMissioni(((MissioneDAO) dataLayer.getDAO(Missione.class)).getMissioniByMezzo(this));
            } catch (DataException ex) {
                Logger.getLogger(MezzoProxy.class.getName()).log(Level.SEVERE, null, ex);
            }
        }
