...
public class GestioneRichiestaAttivaController extends SoccorsoWebBaseController {
        ...
        if (idRichiestaParam != null && !idRichiestaParam.isEmpty()) {
            try {
                ...
                if (richiesta != null) {
                    ...
                    // Se è presente una foto allegata (blob), la codifichiamo in Base64
                    if (richiesta.getFoto() != null && richiesta.getFoto().length > 0) {
                        String base64Foto = Base64.getEncoder().encodeToString(richiesta.getFoto());
                        request.setAttribute("foto_base64", base64Foto);
                    }

                    request.setAttribute("page_title", "Gestione Richiesta Attiva #" + idRichiesta + " - SoccorsoWeb");

                    TemplateResult res = new TemplateResult(getServletContext());
                    res.activate("admin/gestione_richiesta_attiva.html", request, response);
                    return;
                }
            } catch (NumberFormatException ex) {
                Logger.getLogger(GestioneRichiestaAttivaController.class.getName()).log(Level.WARNING, "ID Richiesta non valido: " + idRichiestaParam, ex);
            }
        }

        // Se la richiesta non è valida o non viene trovata, reindirizziamo alla dashboard admin
